...
class Home():
    ...
    def login(self):
        ...
        if c.fetchall():
            ...
            self.tabla.pack()
            self.tabla.bind("<Double-Button-1>",lambda event: self.doubleClick_Agregar_carrito(event))
        ...
        #
            ...
            # BOTONES
            self.agregar_carrito = ttk.Button(frame2, text='A PAGAR')
            self.agregar_carrito.pack()

            self.eliminar_deCarrito = ttk.Button(frame2, text='ELIMINAR DEL CARRITO', command=self.eliminar_prod_carrito)
    ...
    def doubleClick_Agregar_carrito(self,event):

        id_prod= str(self.tabla.item(self.tabla.selection())["values"][0])
        nombre= str(self.tabla.item(self.tabla.selection())["values"][1])
        precio= (self.tabla.item(self.tabla.selection())["values"][2])
        stock= (self.tabla.item(self.tabla.selection())["values"][3])
        descripcion= str(self.tabla.item(self.tabla.selection())["values"][4])
...
#
    ...
    def doubleClick_modificar_cantidad(self, event):

        cantidad = self.tabla_carrito.item(self.tabla_carrito.selection())["values"][3]
    ...
    def ingresar(self):
        ...
        self.tabla_carrito.insert("", tkinter.END,
                                  values=(self.id_prod, self.nombre, self.precio, self.cantidad.get(), self.sTotal))
    ...
    # FUNCION PARA ELIMINAR PRODUCTO DE CARRITO                    Completar
    def eliminar_prod_carrito(self):
        nombre = self.tabla.item(self.tabla.selection())["values"[1]]
